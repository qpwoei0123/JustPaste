...
- 첫 적용 기본 질문은 한 번에 하나씩만 던진다.
- 기본 추천은 `pack 1개 먼저 적용`이다.
- 선택된 pack은 detail/instruction을 재조회해야 한다.
- apply 직전 요약에는 `선택 pack / scope / 예상 변경 / 이번 라운드 비포함 / 위험 / rollback hint`가 모두 포함돼야 한다.
- `ㅇㅇ`, `그거`, `ㄱㄱ`, `진행` 같은 애매한 답변은 승인으로 취급하지 않는다.
- `global` scope는 사용자가 직접 고를 때만 진행한다.
...
- 첫 적용에서는 다음 질문이 예측 가능한 흐름을 우선한다.
...
- extra clarification turns before first dry-run
